...
const startConsumer = async () => {
    ...
    amqp.connect(getRabbitMQUri(), function (error0, connection) {
        ...
        connection.createChannel(function (error1, channel) {
            ...
            channel.assertQueue(RABBITMQ_QUEUES[CONTA_AZUL_SECTIONS.SERVICE], {
                durable: true
            })

            //binding the queue to get messages from domain
            channel.bindQueue(RABBITMQ_QUEUES[CONTA_AZUL_SECTIONS.SERVICE], RABBITMQ_EXCHANGE_TO_LISTEN, 'service');

            console.info(`[*] Waiting for messages in exchange: '${RABBITMQ_EXCHANGE_TO_LISTEN}' --- queue: '${RABBITMQ_QUEUES[CONTA_AZUL_SECTIONS.SERVICE]}'... CTRL+C to exit!`);

            //setting prefetch qty
            channel.prefetch(RABBITMQ_PREFETCH);

            //listening event
            channel.consume(RABBITMQ_QUEUES[CONTA_AZUL_SECTIONS.SERVICE], async function(msg) {
                try {
